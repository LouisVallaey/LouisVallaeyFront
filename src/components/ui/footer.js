  ...
  return (
    <FooterContainer secondary={secondary}>
      <ContentContainer>
        <LeftContainer secondary={!secondary}>
          ...
          <CtaContainer secondary={!secondary}>
            <CtaButton to={`/contact`} key={`contact`}>{cta !== undefined ? cta : 'Contacteer mij'}</CtaButton>
          </CtaContainer>
        </LeftContainer>
        <RouteContainer secondary={!secondary}>
          <RouteList>
            ...
            {pages.slice(0, 2).map((page) => (
              <RouteLink component={Link} to={page.node.link || `/${page.node.name.toLowerCase()}`} key={page.node.id}>{page.node.name}</RouteLink>
            ))}
          </RouteList>
          <RouteList>
            <RouteTitle>About</RouteTitle>
            {pages.slice(2, 4).map((page) => (
              <RouteLink component={Link} to={page.node.link || `/${page.node.name.toLowerCase()}`} key={page.node.id}>{page.node.name === "About" ? "Wie ben ik" : page.node.name}</RouteLink>
            ))}
          </RouteList>
        </RouteContainer>
      </ContentContainer>
      <CreditContainer secondary={!secondary}>
        ...
        <CreditLink >launched by <b>hello.louisvallaey.be</b></CreditLink>
      </CreditContainer>
    </FooterContainer>
  )
}